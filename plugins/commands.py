...
async def start(client, message):
    ...
    if settings.get('is_fsub', IS_FSUB):
        ...
        if btn:
            ...
            await message.reply_photo(
                photo=random.choice(PICS),
                caption=f"👋 Hello {message.from_user.mention},\n\nPlease join my 'Movie Channel' and try again. 😇",
                reply_markup=reply_markup,
                parse_mode=enums.ParseMode.HTML
            )
            return 
...
async def settings(client, message):
    ...
    if settings is not None:
        buttons = [[
            ...
            InlineKeyboardButton('🧲 Button' if settings["links"] else '⛓ Link', callback_data=f'setgs#links#{settings["links"]}#{str(grp_id)}')
        ],[
...
async def plans_cmd_handler(client, message):                
    btn = [            
        [InlineKeyboardButton("Get premium🧾", url="t.me/d347h1_0v3r")],
...
async def check_plans_cmd(client, message):
    ...
    if await db.has_premium_access(user_id):         
        ...
        btn = [ 
            [InlineKeyboardButton("ɢᴇᴛ ғʀᴇᴇ ᴛʀᴀɪʟ ғᴏʀ 30 ᴍɪɴᴜᴛᴇꜱ ☺️", callback_data="get_trail")],
